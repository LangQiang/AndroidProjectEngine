...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void setRecentEmptyView(View emptyView, ViewGroup.LayoutParams params) {
        boolean insert = false;
        if (mEmptyLayout == null) {
            mEmptyLayout = new ScrollView(emptyView.getContext())/* {
                float dx;
                float dy;
                ...
                public boolean onInterceptTouchEvent(MotionEvent ev) {
                    switch (ev.getAction()) {
                        ...
                        case MotionEvent.ACTION_MOVE:
                            float mx = ev.getRawX();
                            float my = ev.getRawY();
                            if (my - dy != 0 && Math.abs((mx - dx) / (my - dy)) < 0.5) {
                                getParent().requestDisallowInterceptTouchEvent(true);
                                return true;
                            } else {
            ...
            try {
                if (params != null) {
                    ...
                    mEmptyLayout.setFillViewport(true); //emptyview 附带params过来的要遵循子view的大小
                    mEmptyLayout.setLayoutParams(new ViewGroup.LayoutParams(params.width, params.height));
                } else {
                    mEmptyLayout.setLayoutParams(new ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
                }
    ...
    public void setEmptyView(View emptyView) {
        ...
        if (mEmptyLayout == null) {
            mEmptyLayout = new ScrollView(emptyView.getContext());
            final ViewGroup.LayoutParams layoutParams = new ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
