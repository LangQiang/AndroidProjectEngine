...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private String pullUpLabel = "上拉获取更多";
	private String loadingLabel = "加载中...";//context.getString(R.string.pull_to_refresh_refreshing_label);
	private String releaseLabel = "可以松开了哦！";//context.getString(R.string.pull_to_refresh_release_label);
	// ===========================================================
	// Constructors
	// ===========================================================
				...
				if (isBeingDragged) {
					...
					if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
						...
						if (currentMode == MODE_PULL_UP_TO_REFRESH) {
							footerLayout.stopDragging(true);
						} else if (currentMode == MODE_PULL_DOWN_TO_REFRESH) {
							headerLayout.stopDragging(true);
						}
						onRefreshListener.onRefresh(currentMode);
					} else {
						if (currentMode == MODE_PULL_UP_TO_REFRESH) {
							footerLayout.stopDragging(false);
						} else if (currentMode == MODE_PULL_DOWN_TO_REFRESH) {
							headerLayout.stopDragging(false);
						}
						smoothScrollTo(0);
					}
					return true;
	...
	protected void setRefreshingInternal(boolean doScroll) {
		...
		if (doScroll) {
			smoothScrollTo(currentMode == MODE_PULL_UP_TO_REFRESH ? headerHeight : -headerHeight);
		}
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_loadingLabel)){
			if (!TextUtils.isEmpty(a.getString(R.styleable.PullToRefresh_loadingLabel))){
				loadingLabel = a.getString(R.styleable.PullToRefresh_loadingLabel);
			}
		}
		if (a.hasValue(R.styleable.PullToRefresh_releaseLabel)){
			if (!TextUtils.isEmpty(a.getString(R.styleable.PullToRefresh_releaseLabel))){
				releaseLabel = a.getString(R.styleable.PullToRefresh_releaseLabel);
			}
		}
		if (a.hasValue(R.styleable.PullToRefresh_pullDownLabel)){
			if (!TextUtils.isEmpty(a.getString(R.styleable.PullToRefresh_pullDownLabel))){
				pullDownLabel = a.getString(R.styleable.PullToRefresh_pullDownLabel);
			}
		}
		if (a.hasValue(R.styleable.PullToRefresh_pullUpLabel)){
			if (!TextUtils.isEmpty(a.getString(R.styleable.PullToRefresh_pullUpLabel))){
				pullUpLabel = a.getString(R.styleable.PullToRefresh_pullUpLabel);
			}
		}

		if (a.hasValue(R.styleable.PullToRefresh_ptr_is_white)){
			isWhite = a.getBoolean(R.styleable.PullToRefresh_ptr_is_white, false);
		}
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel, pullUpLabel, loadingLabel);
...
//				newHeight = Math.round((initialMotionY - lastMotionY) / FRICTION);
	...
	//触发滑动之后 不可以被父view拦截
	protected void setDragLonely(boolean lonely) {
        this.dragLonely = lonely;
    }
}
