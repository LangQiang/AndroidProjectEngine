...
public class BottomTabLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultTextSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 12,
                getResources().getDisplayMetrics());
        // 默认中间控件宽度
        int defaultCenterViewWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 102,
                getResources().getDisplayMetrics());
        // 默认红点大小
        ...
        defaultTextColor = getResources().getColor(R.color.LRLiteBase_cl_white_alpha_50);
        // 获取自定义属性，设置给子控件
        final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.LRLiteBase_BottomTabStyle, 0, 0);
        try {
            mTextSize = ta.getDimensionPixelOffset(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_text_size,
                    defaultTextSize);
            mTextNormalColor = ta.getColor(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_text_normal_color,
                    defaultTextColor);
            mTextSelectColor = ta.getColor(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_text_selected_color,
                    Color.BLACK);
            mIconSize = ta.getDimensionPixelOffset(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_icon_size, 0);
            mCenterViewRes = ta.getResourceId(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_center_view, 0);
            mCenterViewWidth = ta.getDimensionPixelOffset(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_center_view_width,
                    defaultCenterViewWidth);
            mCenterViewPaddingLR = ta.getDimensionPixelOffset(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_center_view_padding_lr,
                    0);
            mRedDotSize = ta.getDimensionPixelOffset(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_red_dot_size,
                    defaultRedDotSize);
            mMessageTextColor = ta.getColor(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_message_tip_text_color,
                    Color.WHITE);
            mMessageTextSize = ta.getDimensionPixelOffset(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_message_text_size,
                    defaultMessageTextSize);
            mMessageBkgColor = ta.getColor(R.styleable.LRLiteBase_BottomTabStyle_bottom_tab_item_message_bkg_color,
                    Color.RED);
        ...
        linePaint.setColor(getResources().getColor(R.color.black6));
...
//                top = (parentBottom - parentTop - view.getMeasuredHeight()) / 2 + parentTop;
                top = dip2px(4) + parentTop;
                bottom = view.getMeasuredHeight() + top;
            } else {
                top = dip2px(24) + parentTop;
                bottom = parentBottom;
            }
...
//                                hideRedDot(mCurrentIndex);
    ...
    public void setCenterViewRes(int id) {
        this.mCenterViewRes = id;
        if (id != 0) {
            isShowCenterView = true;
        }
    }

    public void setCenterViewPaddingLR(int px) {
        this.mCenterViewPaddingLR = px;
    }
