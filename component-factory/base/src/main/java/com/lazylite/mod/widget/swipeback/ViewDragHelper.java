...
public class ViewDragHelper {
     ...
     * {@link swipebackfragment.ViewDragHelper.Callback#tryCaptureView(View, int)}
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    final int idx;
                    final int idy;
                    if (EDGE_TOP == mTrackingEdges || EDGE_BOTTOM == mTrackingEdges){
                        // 为避免有时滑动错位的bug，上下滑动就不要左右动了
                        idx = 0;
                    }else {
                        idx = (int) (x - mLastMotionX[mActivePointerId]);
                    }
                    if (EDGE_RIGHT == mTrackingEdges || EDGE_LEFT == mTrackingEdges) {
                        //为避免有时滑动错位的bug, 左右滑动就不要上下动了
                        idy = 0;
                    }else {
                        idy = (int) (y - mLastMotionY[mActivePointerId]);
                    }
...
//            result |= EDGE_BOTTOM;
        if (x < mParentView.getLeft() + mEdgeSize && EDGE_LEFT == mTrackingEdges) {
            result = EDGE_LEFT;
        } else if (y < mParentView.getTop() + mEdgeSize && EDGE_TOP == mTrackingEdges) {
            result = EDGE_TOP;
        } else if (x > mParentView.getRight() - mEdgeSize && EDGE_RIGHT == mTrackingEdges) {
            result = EDGE_RIGHT;
        } else if (y > mParentView.getBottom() - mEdgeSize && EDGE_BOTTOM == mTrackingEdges) {
            result = EDGE_BOTTOM;
        }
