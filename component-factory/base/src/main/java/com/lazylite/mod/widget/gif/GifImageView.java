package com.lazylite.mod.widget.gif;
...
public class GifImageView extends androidx.appcompat.widget.AppCompatImageView implements Runnable {
    ...
    public int getProgress() {
        List<GifFrame> gifFrames = gifDecoder.getGifFrames();
        if (null == gifFrames) {
            return -1;
        }
        int curIndex = gifDecoder.getCurrentFrameIndex();
        if (curIndex < 0 || curIndex >= gifFrames.size()) {
            return 0;
        }
        int progress = 0;
        for (int i = 0; i < gifFrames.size(); i++) {
            progress += gifFrames.get(i).delay;
            if (i >= curIndex) {
                break;
            }
        }
